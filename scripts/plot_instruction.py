...
def select_samples(
    ...
    if seed is None:
        sampled_keys = random.sample(data, k=count)
    else:
        rng = random.Random(seed)
        sampled_keys = rng.sample(data, k=count)

    return [(item["pair_id"], item) for item in sampled_keys]
...
def main():
    ...
    parser.add_argument(
        "--single_json",
        type=str,
        default="./data/sd_instructions_part0.jsonl",
        help="Path to single dataset instructions.jsonl",
    )
    parser.add_argument(
        "--multi_json",
        type=str,
        default="./data/md_instructions_part0.jsonl",
        help="Path to multi dataset instructions.jsonl",
    )
    parser.add_argument(
        "--seed",
        type=int,
