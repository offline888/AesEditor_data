...
def generate_instruction(model, processor, img_ref_path, img_lq_path, distortion_names, severities):
    ...
    dist_info_text = str(
        ...
        "The first image is produced by adding two kinds of distortion to the second image. "
        "The distortion information and beautification operation information is as follows:\n"
        f"- Distortion type 1: {distortion_name_dict[distortion_names[0]]}\n"
        f"- Severity level 1: {grades[severities[0]-1]} (severity {severities[0]} out of 5)\n"
        f"- Distortion type 2: {distortion_name_dict[distortion_names[1]]}\n"
        f"- Severity level 2: {grades[severities[1]-1]} (severity {severities[1]} out of 5)\n"
    )
    reverse_key = [
        opposite_distortion_name_dict.get(distortion_names[0]),
        opposite_distortion_name_dict.get(distortion_names[1]),
    ]
    if reverse_key[0] and reverse_key[1]:
        dist_info_text+=(
            f"- Beautification operation description:1.{distortion_name_dict[reverse_key[1]]};"
            f"2.{distortion_name_dict[reverse_key[0]]}\n"
        )
        dist_info_text+=(
            f"- Beautification operation grade:1.{bea_grades[severities[1]-1]}(severity {severities[1]} out of 5);"
            f"2.{bea_grades[severities[0]-1]}(severity {severities[0]} out of 5)\n"
        )
    else:
        print(f"Reverse key is not found: {distortion_names}")
    ...
    query_text = str(
        ...
        "5. Each instruction must include both of the aforementioned beautification operations. It should be explained from the perspective of the non‑professional user rather than using the original words.\n"
        "6. The order is not important. You can connect these two beautification operations using words that indicate a parallel relationship.\n"
        "7. Keep each within 40 words.DON'T use useless interjections.\n"
        "8. The instructions are more closely related to daily life and can be presented in a more vivid way.\n\n"
        ...
        "4. Base the instructions on global adjustments rather than specific local areas.\n"
        "5. Each instruction must include both of the aforementioned beautification operations. It should be explained from the perspective of the professional user rather than using the original words.\n"
        "6. The order is not important. You can connect these two beautification operations using words that indicate a parallel relationship.\n"
        "7. Keep each within 40 words.DON'T use useless interjections.\n"
        "Output requirements:\n"
...
if __name__ == "__main__":
    ...
    for image_name, image_data in distortion_data.items():
        img_ref = image_data.get("img_path")
        if not img_ref:
            continue
        ...
        for dist_info in distortions:
            try:
                ...
                distortion_classes = dist_info.get("distortion_classes", [])
                severities = dist_info.get("severities", [])
                distortion_names=[]
                ...
                instructions=generate_instruction(
                        ...
                        img_lq_path=dist_info.get("img_lq"),
